...
WorldStitch automatically backs up notes before any update or delete operation. This document explains how backups work and how to restore them.
...
Copy-Item ".versions\Notes\myfile.md.20260414-143022.bak" "Notes\myfile.md"
```
...
If the server crashes while editing, unsaved content is written to:
...
- All `.ws_meta/` structured model data
- `settings.json`
- `audit.log`
- `ai_usage_log.csv`
...
3. Launch the app — `.ws_meta/` will be regenerated from existing note files on next access
